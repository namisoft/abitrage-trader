...
contract DexArbitrage is Ownable {
    ...
    modifier onlyFundManager() {
        require(isFundManager[msg.sender], "Only fund manager");
        _;
    }
    ...
    constructor() {
        // add contract owner as a fund manager
        isFundManager[msg.sender] = true;
    }
    
    function setDebugMode(bool _isDebugMode) external onlyOwner {
        isDebugMode = _isDebugMode;
    }
    
    function addFundManager(address _toAdd) external onlyOwner {
        isFundManager[_toAdd] = true;
    }
    
    function removeFundManager(address _toRemove) external onlyOwner {
        isFundManager[_toRemove] = false;
    }
    
    function addTrader(address _toAdd) external onlyOwner {
        isTrader[_toAdd] = true;
    }
    
    function removeTrader(address _toRemove) external onlyOwner {
        isTrader[_toRemove] = false;
    }    
    
    function recoverERC20(address _token, address _dest, uint256 _amount) external onlyFundManager {
        _safeTokenTransfer(_token, _dest, _amount);
    }
    
    function approveERC20(address _token, address _spender, uint256 _amount) external onlyFundManager {
    ...
    function tradeOnSingleRouter(
        ...
        address _router,
        uint256 _minProfit,
        uint256 _validToBlockNumber) external onlyTrader {
            require(block.number <= _validToBlockNumber, "1");
            require(IERC20(_token).balanceOf(address(this)) >= _amount, "2");
            ...
            uint256 minAmountOut = _amount + _minProfit;
            
            require(amountOut >= minAmountOut, 
                    isDebugMode ? 
                    string(abi.encodePacked(Strings.toString(amountOut), "/", Strings.toString(minAmountOut))) : 
                    "3"
            );
